...
def time_sampling(
        ...
        n_samp: int = 100,
        graph: bool = False,
        time_data: tuple[datetime64,datetime64] = (
            datetime64('2022-07-17T00:30:00.000000000', 'ns'),
            datetime64('2022-07-31T00:30:00.000000000', 'ns')
            )
        ):
    
    '''This code generates samples at regular intervals for a given xarray dataset and returns 
    an xarray dataset at the required intervals. Note, 'interval' is in seconds.
    ...
    # Generating the random samples, if the sampling type is random. 
    # This returns the randomly sampled list in order of ascending date. 
    ...
    # Generating the structured samples                          
        ...
        if scale_factor.dtype == "timedelta64[ns]":
            ...
            # 10000 is the default value, but if this doesn't work, 
            # set the time increment to the difference between the first and second datapoint. 
        ...
        if graph:
            ...
            time_points = array(arange(n_samp)*interval_ns,dtype='int64') + \
                array([[str(begin_time)]*n_samp],dtype = 'datetime64[ns]')
        else:
            # The interpolation constructs the datetime64 string in order to index the dataset 
            # correctly via the dataset.interp() function
            ...
            initial = datetime64(begin_date[0:4]+'-'+begin_date[4:6]+'-'\
                                     +begin_date[6:8]+'T'+begin_time[0:2]+':'+\
                                        begin_time[2:4],'ns')
            time_points = array(arange(n_samp)*interval_ns, dtype='int64') \
                + array([[str(initial)]*n_samp], dtype = 'datetime64[ns]')
