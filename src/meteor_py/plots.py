"""
This module contains the plotting functions for the weathermodel package.
"""
...
def globalplot(weather_data):
    ...
    if weather_data.wind:
        wind_samples = time_sampling(
            weather_data.wind_data,
            graph = True,
            time_data = (weather_data.date_lower, weather_data.date_upper)
            )
        
    if weather_data.solar:
        solar_samples = time_sampling(
            weather_data.solar_data,
            graph = True,
            time_data = (weather_data.date_lower, weather_data.date_upper)
            )
    ...
    if weather_data.wind:
        ...
        wind_mag    = (wind_mean.variables['U10M'][:,:]**2 +\
                        wind_mean.variables['V10M'][:,:]**2)**0.5
        ...
        print(
            f"Plotting average wind speed data at 10m on time span: "
            f"{str(wind_date_l.values)[0:10]} at {str(wind_date_l.values)[11:16]} "
            f"to {str(wind_date_u.values)[0:10]} at {str(wind_date_u.values)[11:16]}"
        )
        ...
        title(
            f'Average MERRA-2 Wind Speed at 10m \n'
            f'On time span: {str(wind_date_l.values)[0:10]} at {str(wind_date_l.values)[11:16]} '
            f'to {str(wind_date_u.values)[0:10]} at {str(wind_date_u.values)[11:16]}',
            size=14
        )
    ...
    if weather_data.solar:
        ...
        # Extracting the longitude and latitude values, and taking the 
        # values at initial and terminal time. 
        ...
        # These plots can take a while to generate...   
        print(
            f"Plotting average surface solar radiation data on time span: "
            f"{str(solar_date_l.values)[0:10]} at {str(solar_date_l.values)[11:16]} "
            f"to {str(solar_date_u.values)[0:10]} at {str(solar_date_u.values)[11:16]}"
        )
        ...
        clevs = arange(
            0.99*solar_mag[:,:].min(),
            1.01*solar_mag[:,:].max(),
            (solar_mag[:,:].max()-solar_mag[:,:].min())/100
            )
        contourf(
            solar_lon, 
            solar_lat, 
        ...
        title(
            f'Average MERRA-2 Surface Solar Radiation \n'
            f'On time span: {str(solar_date_l.values)[0:10]} at {str(solar_date_l.values)[11:16]} '
            f'to {str(solar_date_u.values)[0:10]} at {str(solar_date_u.values)[11:16]}',
            size=14
        )
        cb = colorbar(
...
def boxplot(
    ...
    if weather_data.wind and not just_solar:
        wind_samples = time_sampling(
            weather_data.wind_data,
            graph = True,
            time_data = (weather_data.date_lower, weather_data.date_upper)
            )
        small_wind_samples = time_sampling(
            weather_data.wind_data_spatial,
            graph = True,
            time_data = (weather_data.date_lower, weather_data.date_upper)
            )
    if weather_data.solar and not just_wind:
        solar_samples = time_sampling(
            weather_data.solar_data,
            graph = True,
            time_data = (weather_data.date_lower, weather_data.date_upper)
            )
        small_solar_samples = time_sampling(
            weather_data.solar_data_spatial,
            graph = True,
            time_data = (weather_data.date_lower, weather_data.date_upper)
            )

    # Finding the bounds of the vertices and the central points to align the map
    extent = [*weather_data.longitudes, *weather_data.latitudes]
    ...
    if weather_data.wind and not just_solar:
        ...
        wind_mag = (wind_mean.variables['U10M'][:,:]**2 + \
                    wind_mean.variables['V10M'][:,:]**2)**0.5
        small_wind_mag = (small_wind_mean.variables['U10M'][:,:]**2 \
                          + small_wind_mean.variables['V10M'][:,:]**2)**0.5
        ...
        print(
            f"Plotting average wind speed data at 10m on time span: "
            f"{str(wind_date_l.values)[0:10]} at {str(wind_date_l.values)[11:16]} to "
            f"{str(wind_date_u.values)[0:10]} at {str(wind_date_u.values)[11:16]}"
            )

        # Generating the plots
        fig = figure(figsize=(8,8))
        ax  = axes(projection=Stereographic(central_lon, central_lat))
        ...
        clevs = arange(
            0.95*small_wind_mag[:,:].min(),
            1.05*small_wind_mag[:,:].max(),
            (small_wind_mag[:,:].max() - small_wind_mag[:,:].min()) / 100
            )
        contourf(
            wind_lon,
            wind_lat,
        ...
        # Points refers to lon/lat coordinates of wind turbines. This allows for the windfarm
        #  to be overlayed on the contour plot
        if points != [(1,1)]:
            scatter(
                [*list(zip(*points))[1]],
                [*list(zip(*points))[0]],
    ...
    if weather_data.solar and not just_wind:
        ...
        print(
            f"Plotting average surface solar radiation data on time span: "
            f"{str(solar_date_l.values)[0:10]} at {str(solar_date_l.values)[11:16]} to "
            f"{str(solar_date_u.values)[0:10]} at {str(solar_date_u.values)[11:16]}"
        )
        ...
        clevs = arange(
            0.95*small_solar_mag[:,:].min(),
            1.05*small_solar_mag[:,:].max(),
            (small_solar_mag[:,:].max()-small_solar_mag[:,:].min())/100
            )
        contourf(
            solar_lon,
            solar_lat,
...
def timeseriesplot(
        self,
        position: tuple[float,float] = (1000,1000),
        xy=([0],[0]),
        title='.',
        xlabel='.',
        ylabel='.',
    ...
    - If you don't specify a position, the default will take an average value accross all points within the 
                                                        square of meterological.longitudes x meterological.latitudes
    - surpess_wind and supress_solar, if set to 'False' will prevent the script from generating plots 
                                                        for wind and solar respectivly

    '''
    if (
        position == (1000, 1000)
        and all(x == 0 for x in xy[0])
        and all(y == 0 for y in xy[1])
    ):
        position = (
            mean(self.latitudes),
            mean(self.longitudes)
            )
        ...
        if self.wind:
            ...
            fig, ax = subplots()
            ax.plot(
                self.wind_data_spatial_temporal.variables['time'],
                mean_wind,
                color = '#800020'
                )
            ax.set(
        ...
        if self.solar:
            ...
            fig, ax = subplots()
            ax.plot(
                self.solar_data_spatial_temporal.variables['time'],
                mean_solar,color = 'navy'
                )
            ax.set(
                xlabel='time',
    ...
    if (
        any(xy[0][i] != 0 for i in range(len(xy[0])))
        or any(xy[1][i] != 0 for i in range(len(xy[1])))
        ):
